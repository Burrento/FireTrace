...
|---|---|---|
| Photo compression and file limits | FR-04, Feature 4, Table 3.13 Photograph Upload | Photographs are resized and compressed on the device before upload; the server accepts JPEG, PNG or WebP up to 5 MB and rejects other files. |
| Hotline on registration, report form and failure notice | FR-01, Business Rule 9, Limitation 4 | As written. |
| Consent at registration | FR-01 | As written. |
| Sign-in audit logging | Feature 25, §S | As written. |
| Linked report shows the canonical incident's status | Limitation 6, Business Rule 6, FR-08 | As written. |
| Rejected / false-alarm outcome | FR-06, Feature 9, Table 3.10 | As written; rejected reports excluded from counts. |
| Unlink and relink | Feature 12 | As written. |
| Manual intake with source channel | FR-06, Feature 8 | As written. |
| Civilian in-system notifications from real status updates | Feature 7 | As written. |
| Downloadable operational report | FR-11, Feature 21 | Downloadable as CSV (not PDF or Excel). |
